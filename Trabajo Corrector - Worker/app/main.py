...
app = FastAPI(title="Trabajo Corrector Worker - PubSub Receiver", version="1.0")
...
def corregir_trabajo(id_centro: str, id_alumno: str, clase: str, asignatura: str, unidad: int, id_trabajo: str, imagen_gcs_path: str):
    
    #Leemos los documentos en los que se basará el trabajo:
    print(f"[PASO 1 INICIADO] Leemos el documento pdf del trabajo | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)

    blob_name=f"{id_centro}/{clase}/{asignatura}/{unidad}/{id_alumno}/{id_trabajo}.pdf"
    document =genai.types.Part.from_uri(
        file_uri=f"gs://{GCS_BUCKET_TRABAJO}/{blob_name}",
        mime_type="application/pdf",
    )
    
    print(f"[PASO 1 FINALIZADO] Leemos el documento pdf del trabajo | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)
    ...
    print(f"[PASO 3 INICIADO] Corrigiendo el trabajo monográfico | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)
    ...
    generate_content_config = genai.types.GenerateContentConfig(
        ...
        response_schema = {"type":"OBJECT","properties":{"nota":{"type":"INTEGER","description":"Nota obtenida por el alumno en el trabajo."},"comentarios_alumno":{"type":"STRING","description":"Comentarios que debe recibir el alumno sobre su desempeño en el trabajo de manera que le ayuden a mejorar. Debe tener una extensión aproximada de {WORDS_COMENTARIO_ALUMNO} palabras."},"comentarios_profesor":{"type":"STRING","description":"Comentarios que debe recibir el profesor del alumno para indicarle en qué cosas se ha equivocado el alumno sobre la resolución del trabajo. Debe tener una extensión aproximada de {WORDS_COMENTARIO_PROFESOR} palabras."}},"required":["nota","comentarios_alumno","comentarios_profesor"]},
    )
    ...
    parts.append(genai.types.Part.from_text(text="Un alumno, al que le habías puesto el siguiente trabajo monográfico:"))
    ...
    parts.append(genai.types.Part.from_text(text="\t 2) Debes evaluar el trabajo monográfico en la medida en la que veas que el alumno se haya esforzado en realizar las tareas propuestas en el propio documento y lograrlas."))
    parts.append(genai.types.Part.from_text(text="\t 3) Proporciona recomendaciones sobre el desempeño del alumno y de que manera puede mejorar si lo consideras oportuno. Si crees que el alumno lo ha hecho muy bien simplemente felicítalo y anímale a seguir adelante en la asignatura recordándole lo importante que es tanto para su yo del presente como futuro."))
    parts.append(genai.types.Part.from_text(text="Importante:"))
    parts.append(genai.types.Part.from_text(text="\t - No corrijas ningun trabajo monográfico que no esté manuscrito. Si alguien te envía una resolución y no está manuscrita asignale una nota de 0 e indícale al alumno que debe resolver al ejercicio con su propia letra."))
    parts.append(genai.types.Part.from_text(text="\t - Evalúa el grado de cumplimiento del trabajo monográfico en la medida en la que el alumno haya realizado las tareas propuestas satisfactoriamente."))
    parts.append(genai.types.Part.from_text(text=f"\t - Identifica, si lo consideras necesario, puntos de mejora en el trabajo monográfico realizado por el alumno. Los comentarios de mejora hacia el alumno debe tener una extensión aproximada de {WORDS_COMENTARIO_ALUMNO} palabras, mientras que el del profesor debe tener aproxiamdamente {WORDS_COMENTARIO_PROFESOR} palabras."))
    parts.append(genai.types.Part.from_text(text=f"\t - Debes usar ** para encapsular aquellos fragmentos en los comentarios al profesor como al alumno para enfatizar lo realmente importante. Por ejemplo, si quieres enfatizar que el alumno no ha realziado del todo bien una tarea, deberías escribir algo como: 'El alumno no ha realizado la tarea **X**'."))
    ...
    print(f"[PASO 3 FINALIZADO] Generando el trabajo monográfico  | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)

    #Actualizamos el contenido generado:

    print(f"[PASO 4 INICIADO] Actualizar el contenido generado | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)

    sql = f"""
                UPDATE `{PROJECT_ID}.{DATASET_ID}.T_CONTENIDO_DISPONIBLE_TRABAJOS`
                SET 
                    nota = @nota,
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            ...
            bigquery.ScalarQueryParameter("id", "STRING", id_trabajo),
    ...
    print(f"[PASO 4 FINALIZADO] Actualizar el contenido generado | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)

    #Registramos la interacción en T_ACCION_ALUMNO_TRABAJO:

    print(f"[PASO 5 INICIADO] Registrar interacción | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)

    sql = f"""
                INSERT INTO `{PROJECT_ID}.{DATASET_ID}.T_ACCION_ALUMNO_TRABAJO`
    ...
    print(f"[PASO 5 FINALIZADO] Registrar interacción | {id_centro} {id_alumno} {clase} {asignatura} {unidad} {id_trabajo}", flush=True)
...
def root():
    ...
    return {
        "service": "Trabajo Corrector Worker",
        "status": "running",
        "version": "1.0",
...
def health():
    ...
    return {"status": "healthy", "service": "trabajo-corrector-worker"}
...
async def task_handler(request: Request):
    ...
    try:
        ...
        action = message_json.get("action", "corregir_trabajo")
        
        if action == "corregir_trabajo":
            # Obtener parámetros
            ...
            id_trabajo = message_json.get("id_trabajo")
            if not id_trabajo:
                raise HTTPException(status_code=400, detail="Missing id_trabajo")
