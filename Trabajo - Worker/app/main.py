...
NO_WORDS_DESAFIO_TITLE=20
NO_WORDS_DESAFIO=150
NO_WORDS_TARGET_TITLE=6
NO_WORDS_TARGET_CONTENT=40
NO_WORDS_OBJECTIVES=40
NO_WORDS_CONSEJO=40
NO_WORDS_FUTURO=40
NO_WORDS_HINT=150
...
HTML_COMPONENTS = {
            "cabecera":{
                    "html":"<div class=\"cabecera-row verde negrita cabecera\"><div class=\"cabecera-left\">#asignatura#</div><div class=\"cabecera-center\">#nivel_academico#</div><div class=\"cabecera-right\"><img class=\"imagen_cabecera\" src=\"https://raw.githubusercontent.com/RafaArmero1993/MentorIA/refs/heads/main/App/images/MentorIA logo b-g.png\"/></div></div>"
            },
            "ejercicio":{
                    "html":"<div class=\"ejercicio negro\">#content#</div>",
                    "text_lengths":[1400]
            },
            "qr":{
                    "html":"<div class=\"qr_panel\"><div class=\"qr_panel__img\"><img src=\"data:image/png;base64,#qr_image#\"></div><div class=\"qr_panel__text negrita negro\">Accede a este enlace<br/>para poder obtener<br/>una pista</div></div><br><br><br><br><br>",
                },
            "unidad":{
                    "html":"<h1 class=\"unidad verde negrita\">#content#</h1>"
                },
            "capitulo":{
                    "html":"<h2 class=\"capitulo verde negrita\">#content#</h2>"
                },
            "seccion":{
                    "html":"<h3 class=\"seccion verde negrita\">#content#</h3>"
                },
}
...
app = FastAPI(title="Trabajo Worker - PubSub Receiver", version="1.0")
...
def generar_trabajo(id_centro: str, id_alumno: str, clase: str, asignatura: str, unidad: int):
    ...
    print(f"[PASO 2 INICIADO] Leemos los intereses del alumno | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    print(f"[PASO 2 FINALIZADO] Leemos los intereses del alumno | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #Procesamos los intereses del alumno:
    print(f"[PASO 3 INICIADO] Procesamos los intereses del alumno | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    print(f"[PASO 3 FINALIZADO] Procesamos los intereses del alumno | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #Leemos los documentos en los que se basará el ejercicio:
    print(f"[PASO 4 INICIADO] Leemos los documentos PDF generados | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    documents=[]
    for idx, row in secciones.iterrows():
        
        print(f"[PASO 4 PROCESANDO] Step {idx+1}/{len(secciones)} | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    print(f"[PASO 4 FINALIZADO] Leemos los documentos PDF generados | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #Tras esto generamos el ejercicio:
    print(f"[PASO 5 INICIADO] Generando el ejercicio | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    generate_content_config = genai.types.GenerateContentConfig(
        ...
        response_schema = {
            "type":"OBJECT",
            "properties":{
                "microproyecto":{"type":"STRING","description":f"Enunciado del microproyecto que el alumno debe resolver. Debe tener una extension aproximada de {NO_WORDS_DESAFIO} palabras."},
                "microproyecto_title":{"type":"STRING","description":f"Titulo del microproyecto que el alumno debe resolver. Debe tener una extension aproximada de {NO_WORDS_DESAFIO_TITLE} palabras."},
                "title_tarjeta_1":{"type":"STRING","description":f"Titulo de la primera actividad a realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_TITLE} palabras"},
                "content_tarjeta_1":{"type":"STRING","description":f"Descripcion de la primera actividad que el alumno debe realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_CONTENT} palabras."},
                "title_tarjeta_2":{"type":"STRING","description":f"Titulo de la segunda actividad a realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_TITLE} palabras"},
                "content_tarjeta_2":{"type":"STRING","description":f"Descripcion de la segunda actividad que el alumno debe realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_CONTENT} palabras."},
                "title_tarjeta_3":{"type":"STRING","description":f"Titulo de la tercera actividad a realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_TITLE} palabras"},
                "content_tarjeta_3":{"type":"STRING","description":f"Descripcion de la tercera actividad que el alumno debe realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_CONTENT} palabras."},
                "title_tarjeta_4":{"type":"STRING","description":f"Titulo de la cuarta actividad a realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_TITLE} palabras"},
                "content_tarjeta_4":{"type":"STRING","description":f"Descripcion de la cuarta actividad que el alumno debe realizar. Debe tener una extension aproximada de {NO_WORDS_TARGET_CONTENT} palabras."},
                "objetivos":{"type":"STRING","description":f"Objetivos que debe lograr el alumno con la resolucion del microproyecto. Debe tener una extension aproximada de {NO_WORDS_OBJECTIVES} palabras."},
                "consejo":{"type":"STRING","description":f"Consejos practicos para afrontar el microproyecto. Debe tener una extension aproximada de {NO_WORDS_CONSEJO} palabras."},
                "futuro":{"type":"STRING","description":f"Aplicabilidad futura de la actividad para el alumno en su dia a dia a futuro. Debe tener una extension aproximada de {NO_WORDS_FUTURO} palabras."}
            },
            "required":["microproyecto","microproyecto_title","title_tarjeta_1","content_tarjeta_1","title_tarjeta_2","content_tarjeta_2","title_tarjeta_3","content_tarjeta_3","title_tarjeta_4","content_tarjeta_4","objetivos","consejo","futuro"]
        },
    )

    parts=[]
    parts.append(genai.types.Part.from_text(text=f"Eres un orientador formativo que crea microproyectos a sus alumnos para demostrarles la aplicabilidad real del contenido de la asignatura {asignatura} a los alumnos de la clase {clase}."))
    parts.append(genai.types.Part.from_text(text=f"Uno de tus alumnos te pide que le elabores un microproyecto para que pueda ver la utilidad práctica del contenido que se explica en la unidad {unidad} de la asignatura {asignatura}."))
    ...
    if not intereses_txt == "":
        parts.append(genai.types.Part.from_text(text="El alumno te pide que en la medida de lo posible elabores un microproyecto que tenga en cuenta sus intereses y para ello decides revisar un formulario que se les envió a principio de curso en el que el alumno respondió a varias preguntas sobre sus intereses, motivaciones, frustraciones y asignaturas que se le dan mejor. El contenido de dicho formulario es el siguiente:"))
        parts.append(genai.types.Part.from_text(text=intereses_txt))
    parts.append(genai.types.Part.from_text(text="Cada microproyecto que realizas se compone de:"))
    parts.append(genai.types.Part.from_text(text=f"\t - Un enunciado del microproyecto que el alumno debe resolver. Este enunciado debe ser breve, claro y conciso, y debe tener una extensión aproximada de {NO_WORDS_DESAFIO} palabras."))
    parts.append(genai.types.Part.from_text(text=f"\t - Cuatro actividades que deben ser realizadas secuencialmente para la consecución del microproyecto que el alumno debe resolver. Estas actividades cuentan con un título debe ser breve, claro y conciso, y debe tener una extensión aproximada de {NO_WORDS_TARGET_TITLE} palabras y una descripción detallada de la actividad a realizar que debe ser clara y concisa y tener una extensión aproxiamda de {NO_WORDS_TARGET_CONTENT} palabras."))
    parts.append(genai.types.Part.from_text(text=f"\t - Un apartado con los objetivos del microproyecto a realizar y que tiene que ser claro y conciso y tener uan extensión aprocimada de {NO_WORDS_OBJECTIVES} palabras."))
    parts.append(genai.types.Part.from_text(text=f"\t - Un apartado con consejos para afrontar el microproyecto. Esta utilidad tiene que ser clara y concisa y tener uan extensión aprocimada de {NO_WORDS_CONSEJO} palabras. Debe estar compuesto pro entre 3-5 consejos prácticos que el alumno pueda aplicar para la realización del microproyecto."))
    parts.append(genai.types.Part.from_text(text=f"\t - Un apartado con la utilidad a futuro de los contenidos explicados en la unidad {unidad} de la asignatura {asignatura}  en su día a día, y los cuales se pretende fortalecer con la realización del microproyecto. Esta utilidad tiene que ser clara y concisa y tener uan extensión aprocimada de {NO_WORDS_FUTURO} palabras. Debe estar compuesto por 3-5 aplicaciones reales a futuro prácticas."))
    ...
    if not intereses_txt == "":
        ...
        parts.append(genai.types.Part.from_text(text="\t 3) Establecer un microrpoyecto que pueda se rresuelto por el usuario y que combine tanto sus intereses personales como el contneido de la asignatura. Debe ser algo fácil, ameno pero a la vez motivador."))
        parts.append(genai.types.Part.from_text(text="\t 4) Describir cuales son los objetivos principales de dicho microproyecto."))
        parts.append(genai.types.Part.from_text(text="\t 5) Proporcionar entre 3 y 5 consejos para la resolución del microproyecto."))
        parts.append(genai.types.Part.from_text(text="\t 6) Listar con, entre 3 y 5 utilidades prácticas que el alumno podría obtener a futuro al fortalecer los contenidos explicados en la unidad {unidad} de la asignatura {asignatura} con la realización del microproyecto."))
    else:
        parts.append(genai.types.Part.from_text(text="\t 3) Establecer un microrpoyecto que pueda se rresuelto por el usuario y que combine tanto sus intereses personales como el contneido de la asignatura. Debe ser algo fácil, ameno pero a la vez motivador."))
        parts.append(genai.types.Part.from_text(text="\t 4) Describir cuales son lso objetivos principales de dicho microproyecto."))
        parts.append(genai.types.Part.from_text(text="\t 5) Proporcionar entre 3 y 5 consejos para la resolución del microproyecto."))
        parts.append(genai.types.Part.from_text(text="\t 6) Listar con, entre 3 y 5 utilidades prácticas que el alumno podría obtener a futuro al fortalecer los contenidos explicados en la unidad {unidad} de la asignatura {asignatura} con la realización del microproyecto."))
    parts.append(genai.types.Part.from_text(text="Importante:"))
    parts.append(genai.types.Part.from_text(text="- Los diferentes componentes que conforman el microproyecto deben ser breves, claros y concisos, por lo que EVITA SALUDARLE Y VE AL GRANO."))
    ...
    parts.append(genai.types.Part.from_text(text="- Proporciona cada componente usando HTML que únicamente utilice las etiquetas <p>, <b>, <ul> y <li>). No incluyas ningún otro tipo de etiqueta HTML ni estilos CSS."))
    ...
    microproyecto_title = output.get("microproyecto_title")
    microproyecto = output.get("microproyecto")
    title_tarjeta_1 = output.get("title_tarjeta_1")
    content_tarjeta_1 = output.get("content_tarjeta_1")
    title_tarjeta_2 = output.get("title_tarjeta_2")
    content_tarjeta_2 = output.get("content_tarjeta_2")
    title_tarjeta_3 = output.get("title_tarjeta_3")
    content_tarjeta_3 = output.get("content_tarjeta_3")
    title_tarjeta_4 = output.get("title_tarjeta_4")
    content_tarjeta_4 = output.get("content_tarjeta_4")
    objetivos = output.get("objetivos")
    futuro = output.get("futuro")
    consejo = output.get("consejo")

    print(f"[PASO 5 FINALIZADO] Generando el ejercicio | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #Construimos el ejercicio final creando un documento HTML y guardándolo en GCS:

    print(f"[PASO 6 INICIADO] Construyendo el ejercicio final en HTML y guardándolo en GCS | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    
    html_content=f"<!doctype html><html lang=\"es\"><head>{CSS_RULES}</head><body><div class=\"dina4\">"

    html_content+=f"""
    <section class="desafio">
    <h2 class="desafio__title">{microproyecto_title}</h2>
    <p class="desafio__text justificado">{microproyecto}</p>
    <h3 class="desafio__subtitle">Cuatro actividades a realizar:</h3>
    <div class="desafio__grid">
    <article class="scenario scenario--blue"><div class="scenario__head"><h4 class="scenario__title">{title_tarjeta_1}</h4></div><p class="scenario__desc justificado">{content_tarjeta_1}</p></article>
    <article class="scenario scenario--green"><div class="scenario__head"><h4 class="scenario__title">{title_tarjeta_2}</h4></div><p class="scenario__desc justificado">{content_tarjeta_2}</p></article>
    <article class="scenario scenario--yellow"><div class="scenario__head"><h4 class="scenario__title">{title_tarjeta_3}</h4></div><p class="scenario__desc justificado">{content_tarjeta_3}</p></article>
    <article class="scenario scenario--pink"><div class="scenario__head"><h4 class="scenario__title">{title_tarjeta_4}</h4></div><p class="scenario__desc justificado">{content_tarjeta_4}</p></article>
    </div>
    </section>
    """

    html_content+=f"""<section class="entrega">
    <h2 class="entrega__title">¿Cómo entregar tu trabajo?</h2>
    <p class="entrega__subtitle">Elige el formato que prefieras para presentar tu análisis:</p>

    <div class="entrega__cards">
      <article class="entrega__card">
        <div class="entrega__icon" aria-hidden="true">
          <!-- lápiz -->
          <svg viewBox="0 0 24 24" width="34" height="34" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 20h4l11-11a2.1 2.1 0 0 0 0-3l-1-1a2.1 2.1 0 0 0-3 0L4 16v4z" stroke="#7EB900" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M13 6l5 5" stroke="#7EB900" stroke-width="2" stroke-linecap="round"/>
          </svg>
        </div>
        <h3 class="entrega__card-title">Respuestas escritas</h3>
        <p class="entrega__card-text">Redacta tus respuestas en formato de texto explicando la realización de cada actividad</p>
      </article>
      ...
      <article class="entrega__card">
        <div class="entrega__icon" aria-hidden="true">
          <!-- tabla -->
          <svg viewBox="0 0 24 24" width="34" height="34" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1z" stroke="#7EB900" stroke-width="2"/>
            <path d="M3 10h18M9 5v14M15 5v14" stroke="#7EB900" stroke-width="2" stroke-linecap="round"/>
          </svg>
        </div>
        <h3 class="entrega__card-title">Diagrama o tabla</h3>
        <p class="entrega__card-text">Crea una tabla comparativa o diagrama visual con tus hallazgos</p>
      </article>
      ...
      <article class="entrega__card">
        <div class="entrega__icon" aria-hidden="true">
          ...
          <svg viewBox="0 0 24 24" width="34" height="34" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1z" stroke="#7EB900" stroke-width="2"/>
            <path d="M8.5 10a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" fill="#7EB900"/>
            <path d="M21 16l-5-5-4 4-2-2-6 6" stroke="#7EB900" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </div>
        <h3 class="entrega__card-title">Infografía simple</h3>
        <p class="entrega__card-text">Diseña una infografía visual que muestre tus análisis de forma creativa</p>
      </article>
    </div>
    """
    ...
    <div class="info-box">
      <div class="info-box__icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="12" cy="12" r="9" stroke="#1f1f1f" stroke-width="2" opacity=".75"/>
          <circle cx="12" cy="12" r="4" stroke="#1f1f1f" stroke-width="2" opacity=".75"/>
          <circle cx="12" cy="12" r="1.4" fill="#1f1f1f" opacity=".75"/>
        </svg>
      </div>
      <div>
        <p class="info-box__title">Objetivo del proyecto</p>
        <p class="info-box__text justificado">{objetivos}</p>
      </div>
    </div>
    """
    ...
    <div class="info-box">
      ...
      <div>
        <p class="info-box__title">Aplicación futura del proyecto</p>
    ...
    bucket = storage_client.bucket(GCS_BUCKET_TRABAJO)
    blob_path = f"{id_centro}/{clase}/{asignatura}/{unidad}/{id_alumno}/{1}.html"
    blob = bucket.blob(blob_path)
    blob.upload_from_string(html_content, content_type="text/html; charset=utf-8")

    print(f"[PASO 6 FINALIZADO] Construyendo el ejercicio final en HTML y guardándolo en GCS | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    # Ahora lo que hacemos es guardarlo en formato PDF:

    print(f"[PASO 7 INICIADO] Construyendo el ejercicio final en PDF y guardándolo en GCS | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    pdf_bytes = WeasyHTML(string=html_content).write_pdf()
    bucket = storage_client.bucket(GCS_BUCKET_TRABAJO)
    pdf_blob_path = f"{id_centro}/{clase}/{asignatura}/{unidad}/{id_alumno}/{1}.pdf"
    pdf_blob = bucket.blob(pdf_blob_path)
    pdf_blob.upload_from_string(pdf_bytes, content_type='application/pdf')

    print(f"[PASO 7 FINALIZADO] Construyendo el ejercicio final en PDF y guardándolo en GCS | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    # Lo siguiente que hacemos es generar un "hint":

    print(f"[PASO 8 INICIADO] Generando un hint para el ejercicio | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    parts=[]
    parts.append(genai.types.Part.from_text(text=f"Eres un profesor de la asignatura {asignatura} para los alumnos de la clase {clase}."))
    parts.append(genai.types.Part.from_text(text=f"El otro día le pusiste el siguiente microproyecto a uno de tus alumnos:"))
    parts.append(genai.types.Part.from_bytes(data=pdf_bytes,mime_type="application/pdf"))
    parts.append(genai.types.Part.from_text(text=f"Ahora, ese alumno te pide que lo evalues y les des consejos tanto sobre como realizar el microproyecto ya que no puede hacerlo por su propia cuenta."))
    ...
    if not intereses_txt == "":
        ...
        parts.append(genai.types.Part.from_text(text=intereses_txt))
    parts.append(genai.types.Part.from_text(text="Tu tarea:"))
    parts.append(genai.types.Part.from_text(text="\t 1) Leer atentamente el microproyecto que le pusiste al alumno."))
    ...
    parts.append(genai.types.Part.from_text(text="- No debes darle la resolución dle microproyecto, únciamente debes guiar al alumno para que lo resuelva."))
    ...
    print(f"[PASO 8 FINALIZADO] Hint generado para el ejercicio | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #El siguiente paso será formatear el texto para evitar que contenga caracteres no legibles:

    print(f"[PASO 9 INICIADO] Formatear el hint | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    print(f"[PASO 9 FINALIZADO] Formatear el hint | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #Tras esto intentamso enfatizar el mensaje por audio añadiendo pequeñas tips para la resolución:

    print(f"[PASO 10 INICIADO] Enfatizar el hint | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    print(f"[PASO 10 FINALIZADO] Enfatizar el hint | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #Tras esto generamos el audio y lo gaurdamso en GCP:

    print(f"[PASO 11 INICIADO] Generar el audio y almacenarlo en GCP | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
    ...
    bucket = storage_client.bucket(GCS_BUCKET_TRABAJO)
    blob_path = f"{id_centro}/{clase}/{asignatura}/{unidad}/{id_alumno}/{1}.mp3"
    blob = bucket.blob(blob_path)
    blob.upload_from_string(audio_data, content_type='audio/mpeg')

    print(f"[PASO 11 FINALIZADO] Generar el audio y almacenarlo en GCP | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    #Actualizamos el contenido generado:

    print(f"[PASO 12 INICIADO] Actualizar el contenido generado | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)

    sql = f"""
                INSERT INTO `{PROJECT_ID}.{DATASET_ID}.T_CONTENIDO_DISPONIBLE_TRABAJOS`
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            ...
            bigquery.ScalarQueryParameter("asignatura", "STRING", asignatura),
            bigquery.ScalarQueryParameter("unidad", "INT64", unidad),
            bigquery.ScalarQueryParameter("id", "STRING", str(1)),
    ...
    print(f"[PASO 12 FINALIZADO] Actualizar el contenido generado | {id_centro} {id_alumno} {clase} {asignatura} {unidad}", flush=True)
...
def health():
    ...
    return {"status": "healthy", "service": "trabajo-worker"}
...
async def task_handler(request: Request):
    ...
    try:
        ...
        action = message_json.get("action", "generar_trabajo")
