...
export default function DetalleMicroproyectoAlumno() {
  ...
  const [loadingMicroproyectos, setLoadingMicroproyectos] = useState(true);

  // Datos del microproyecto
  const [microproyectos, setMicroproyectos] = useState<any[]>([]);
  const [microproyectoActual, setMicroproyectoActual] = useState(0); // Índice del microproyecto actual (0-based)
  ...
  const handleGenerarNuevo = () => {
    if (!centroId || !alumnoId || !clase || !asignatura || !unidad) {
      alert("Faltan datos para generar el microproyecto");
      return;
    }

    // Llamar al backend para crear Cloud Task
    fetch(`${import.meta.env.VITE_BASEDATOS_BASE}/generar_trabajo`, {
      method: "POST",
      headers: { 
      ...
      .then(res => {
        if (res.success) {
          ...
          alert("Error al generar el microproyecto. Por favor, inténtalo de nuevo.");
        }
      })
      .catch(err => {
        console.error(err);
        alert("Error de conexión al generar el microproyecto.");
      });
  };
  ...
  useEffect(() => {
    ...
    me(jwt).then(data => {
      ...
      // Cargar microproyectos disponibles desde el backend
      if (clase && asignatura && unidad && data.centro_id) {
        setLoadingMicroproyectos(true);
        fetch(`${import.meta.env.VITE_BASEDATOS_BASE}/listar_microproyectos_alumno`, {
          method: "POST",
          headers: { 
            "Content-Type": "application/json", 
            "x-api-key": import.meta.env.VITE_BASEDATOS_API_KEY 
          ...
          .then(r => r.json())
          .then(res => {
            if (res.microproyectos && res.microproyectos.length > 0) {
              setMicroproyectos(res.microproyectos);
              setMicroproyectoActual(0); // Mostrar el primer microproyecto
              setTituloUnidad(res.titulo_unidad || `Unidad ${unidad}`);
            } else {
              setMicroproyectos([]);
              setTituloUnidad(res.titulo_unidad || `Unidad ${unidad}`);
            }
            setLoadingMicroproyectos(false);
          })
          .catch(() => {
            setMicroproyectos([]);
  ...
  const handlePista = () => {
    if (!microproyectos[microproyectoActual]?.url_audio_hint) {
      alert("No hay audio de pista disponible para este microproyecto");
      return;
    }
    ...
    if (isPlayingAudio && audioRef.current) {
      ...
      const audio = new Audio(microproyectos[microproyectoActual].url_audio_hint);
  ...
  const handleEnviarCorreccion = () => {
    if (!imagenCorreccion) {
      alert("Por favor, carga una imagen del microproyecto resuelto");
      return;
    }
    ...
    reader.onloadend = () => {
      ...
      fetch(`${import.meta.env.VITE_BASEDATOS_BASE}/corregir_trabajo`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json", 
          "x-api-key": import.meta.env.VITE_BASEDATOS_API_KEY 
        ...
        body: JSON.stringify({ 
          ...
          asignatura: decodeURIComponent(asignatura),
          unidad: parseInt(unidad),
          id_trabajo: microproyectos[microproyectoActual]?.id,
        ...
        .then(res => {
          if (res.success) {
            ...
            alert("Error al enviar el microproyecto para corrección. Por favor, inténtalo de nuevo.");
          }
        })
        .catch(err => {
          console.error(err);
          alert("Error de conexión al enviar el microproyecto.");
        });
    };
  ...
  return (
    <div style={{ fontFamily: "system-ui", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <nav style={{
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "8px", padding: "0 16px" }}>
            <button 
              onClick={() => nav(`/microproyectos-asignatura/${clase}/${asignatura}`)} 
              style={{ 
                width: "100%", 
                padding: "12px 16px", 
                border: "none", 
          ...
          {loading ? (
            ...
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%", flexDirection: "column" }}>
              ...
              <p style={{ fontSize: 16, color: "#9ca3af", margin: "0 0 24px 0" }}>
                No hay microproyectos disponibles para esta unidad
              </p>
              <button
            ...
            <div style={{ flex: 1, display: "flex", gap: 24, overflow: "hidden", minHeight: 0 }}>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  {microproyectos[microproyectoActual]?.url_html ? (
                    <iframe 
                      src={microproyectos[microproyectoActual].url_html}
                      style={{
                        width: "100%",
                        height: "100%",
                        border: "none",
              ...
              <div style={{ 
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
                  ...
                  <div style={{
                    ...
                    {microproyectos[microproyectoActual]?.comentarios_alumno ? (
                      renderTextWithBold(microproyectos[microproyectoActual].comentarios_alumno)
                    ) : (
      ...
      {/* Modal de generación de microproyecto */}
      {showGeneratingModal && (
        <div 
          ...
          <div 
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ 
                ...
                Sube una imagen del microproyecto resuelto:
              </label>
